					...
					<div class="topbar__item">
						<div class="topbar-dropdown">
							<button class="topbar-dropdown__btn font-weight-bold text-dark"
								type="button">
								<p class="text-primary">${nhanvien.getHoTen()}</p>
								<button class="topbar-dropdown__btn font-weight-bold text-dark"
									id="dangxuatnhanvien" type="button">Đăng xuất
						</div>
					</div>
				</c:if>
			</div>
							...
							<div class="col-xl-7 col-lg-6 col-md-8">
								<div class="card flex-grow-1 mb-0 mt-5">
									<div class="card-body">
										...
										<form id="formThemSanPham" onsubmit="return false">
											<div class="form-group">
												<label for="track-order-id">Tên Sản Phẩm</label> <input
													name="tensanphamadd" id="tensanphamadd" type="text"
													class="form-control" placeholder="Tên Sản Phẩm">
											</div>
											<div class="form-group">
												<label for="track-order-id">Giá tiền</label> <input
													id="giatiemadd" type="number" class="form-control"
													name="giatiemadd" placeholder="Giá tiền">
											</div>
											<div class="form-group">
												<label for="track-order-id">Mô tả</label>
												<textarea class="form-control" type="text" id="motasanpham"
													name="motasanpham" rows="8" cols="50"></textarea>
											</div>
											<div class="form-group">
												<label for="track-order-id">Hình ảnh</label> <input
													type="file" class="form-control" style="border: none"
													name="hinhanhadd" accept='image/*'>

											</div>
											<div class="form-group">
												<label for="track-order-id">Số lượng</label> <input
													id="soluongadd" type="number" class="form-control"
													name="soluongadd" placeholder="Số lượng">
											</div>
											<div class="form-group">
												<label for="track-order-id">Danh mục</label> <select
													class="form-control" id="madanhmucthemsanpham"
													style="height: 35px" name="madanhmucthemsanpham">
													<option value="default">Chọn danh mục sản phẩm</option>
													<c:forEach var="danhmuc" items="${danhmuc}">
														<option value="${danhmuc.getMaDanhMucSanPham()}">${danhmuc.getTenDanhMuc()}</option>
													</c:forEach>
												</select>
											</div>
											<div class="pt-3">
												<button type="submit" class="btn btn-success login"
													id="btnThemSanPham">Lưu</button>
											</div>
										</form>
									</div>
								</div>
